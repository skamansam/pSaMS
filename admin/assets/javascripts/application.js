! function($) {
  ...
  $(function() {
    function toggleAction(selector, disabled) {
        ...
        $(selector)[method]('list-menu-link-disabled').parent()[method](
    ...
    if (list.length > 0) {
        ...
        $(this).addClass('list-row-action-wrapper-link-active')
          ...
          .find('.cancel').on('click', function() {
            ...
            $(this).parents('.list-row-action-popover-delete-one').hide()
              .siblings('.list-row-action-delete-one').removeClass(
      ...
      $('#delete-selected').on('click', function(ev) {
        ...
        $(this).parent().addClass('active').parent('.dropdown').addClass(
          'open');
        $(this).addClass('active')
          ...
          .find('.cancel').on('click', function() {
            ...
            $(this).parents('.list-menu-popover-delete-selected').hide()
              .siblings('#delete-selected').removeClass('active').parent()
              .removeClass('active');
            // and close the dropdown
            $(this).parents('.dropdown').removeClass('open');
          });

        $(this).siblings('.list-menu-popover-delete-selected').find(
          ':hidden[data-delete-many-ids=true]').
        val(listCheckboxes.filter(':checked').map(function() {
          return $(this).val();
        }).toArray().join(','));
      });
      ...
      listCheckboxes.on('click', function(ev) {
        ...
        $(this).parent('.list-row')[$(this).is(':checked') ? 'addClass' :
    ...
    var error_input;
    if (error_input = $('.has-error :input').first()) {
      error_input.focus();
    }
    ...
    $('select#post_editor_select').on('change', function() {
      var selected = $(this).val();
      var editor = $('.post-editors .post-editor[name="' + selected +
        '"]');
      console.log(editor);
